...
LobbyManager.prototype.joinRoom = function(roomName, joinerId, isOwner) {
  ...
  if (this.players[joinerId]) {
    if (this.rooms[roomName]) {
      ...
      if (player.status == Constants.STATUS_IN_LOBBY) {
        player.status = Constants.STATUS_IN_ROOM;
 ...
 * @return {Object}
 */
LobbyManager.prototype.leaveRoom = function(leaverId) {
  if (this.players[leaverId]) {
    this.players[leaverId].status = Constants.STATUS_IN_LOBBY;
    var roomNames = Object.keys(this.rooms);
    for (var i = 0; i < roomNames.length; ++i) {
      if (this.rooms[roomNames[i]][leaverId]) {
        delete this.rooms[roomNames[i]][leaverId];
      }
      if (Object.keys(this.rooms[roomNames[i]]).length == 0) {
        delete this.rooms[roomNames[i]];
        break;
      }
    }
    return {
...
LobbyManager.prototype.sendState = function() {
  ...
  console.log(this.players, this.rooms);
};
